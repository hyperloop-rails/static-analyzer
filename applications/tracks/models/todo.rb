...
def self.deferred_or_blocked

end

def self.not_deferred_or_blocked
...
def self.due_today
where("todos.due <= ?", Time.zone.now)
end

def self.with_tag
lambda { |tag_id| joins("INNER JOIN taggings ON todos.id = taggings.taggable_id").where("taggings.tag_id = ? AND taggings.taggable_type='Todo'", tag_id) }
end

def self.with_tags
lambda { |tag_ids| where("EXISTS(SELECT * from taggings t WHERE t.tag_id IN (?) AND t.taggable_id=todos.id AND t.taggable_type='Todo')", tag_ids) }
end

def self.completed_after
lambda { |date| where("todos.completed_at > ?", date) }
end

def self.completed_before
lambda { |date| where("todos.completed_at < ?", date) }
end

def self.created_after
lambda { |date| where("todos.created_at > ?", date) }
end
...
def self.created_or_completed_after
lambda { |date| where("todos.created_at > ? or todos.completed_at > ?", date, date) }
end
