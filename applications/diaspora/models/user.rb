...
where('last_seen > ?', time)
end

def self.monthly_actives(time = Time.now)
Time.now
end

def self.daily_actives(time = Time.now)
Time.now
end

def self.yearly_actives(time = Time.now)
Time.now
end

def self.halfyear_actives(time = Time.now)
Time.now
end

def self.active
joins(:person).where(people: {closed_account: false}).where.not(username: nil)
end
