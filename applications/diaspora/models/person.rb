...
def self.remote
where('people.owner_id IS NULL')
end

def self.local
where('people.owner_id IS NOT NULL')
end
...
def self.all_from_aspects(aspect_ids, user)
joins(:contacts => :aspect_memberships).
         where(:contacts => {:user_id => user.id}).
         where(:aspect_memberships => {:aspect_id => aspect_ids})
end

def self.unique_from_aspects(aspect_ids, user)
all_from_aspects(aspect_ids, user).select('DISTINCT people.*')
end

def self.in_aspects(aspect_ids)
joins(:contacts => :aspect_memberships).
        where(:aspect_memberships => {:aspect_id => aspect_ids})
end

def self.profile_tagged_with(tag_name)
joins(:profile => :tags)
      .where(:tags => {:name => tag_name})
      .where('profiles.searchable IS TRUE')
end

def self.who_have_reshared_a_users_posts(user)
joins(:posts)
