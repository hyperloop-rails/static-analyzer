  class Order < ActiveRecord::Base
    ...
    def create_payment_shipment
      ...
      if shipment_line_item.nil?
        new_shipment_line_item = LineItem.new({ :line_item_type => "shipment" })
      ...
      if !self.line_items.detect { |li| li.line_item_type == "payment" }
        payment_line_item = LineItem.new({ :line_item_type => "payment" })
    ...
    def add_line_items(cart)
      ...
      cart.sellables.each do |item|
        self.line_items << LineItem.new({ :sellable_id => item[:sellable].id,
