  class LineItem < ActiveRecord::Base
    ...
    def preprocess_sellable
      ...
      sellable = Sellable.where(id: self.sellable_id).first
    ...
    def preprocess_payment
      if self.new_record?
        ...
        self.payment.payment_method_id ||= PaymentMethod.where(active: true).first.id
    ...
    def sellable_id_enum
      Sellable.all.collect { |s| ["#{s.description}: $#{s.price}", s.id ] }
    end
    ...
    def self.sorted_line_item_types
      Config.line_item_types.sort { |a, b| (a[1][:sort] || 100) <=> (b[1][:sort] || 100) }.collect { |a| a[0] }
    end
  end
