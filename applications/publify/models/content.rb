...
def self.published
where(published: true, published_at: Time.at(0)..Time.now).order('published_at DESC')
end

def self.published_at
published.where(published_at: PublifyTime.delta(*time_params)).order('published_at DESC')
end
...
def self.no_draft
where('state <> ?', 'draft').order('published_at DESC')
end

def self.searchstring
