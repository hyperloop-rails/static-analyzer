...
class Article < Content
  ...
  has_many :comments, -> { order('created_at ASC') }, dependent: :destroy do
    ...
    def self.drafts
where(state: 'draft').order('created_at DESC')
end
...
def self.published_since
published.where('published_at > ?', time).order('published_at DESC')
end

def self.withdrawn
where(state: 'withdrawn').order('published_at DESC')
end

def self.pending
where('state = ? and published_at > ?', 'publication_pending', Time.now).order('published_at DESC')
end
